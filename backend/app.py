...
CRYPTOCOMPARE_URL    = os.getenv("CRYPTOCOMPARE_URL", "https://min-api.cryptocompare.com/data/v2/histominute")
...
log = logging.getLogger("cryptoscalp")
...
model_lock      = threading.Lock()   # thread-safe model inference
model_cache     = {}
signal_cache    = {}
cache_timestamps = {}  # for TTL eviction
...
# ── Cache management ───────────────────────────────────────────
def _cache_set(key, model, scaler, signal_data):
    """Thread-safe cache set with timestamp."""
    with cache_lock:
        # Evict oldest if at max capacity
        if len(model_cache) >= MAX_CACHE_ENTRIES:
            oldest = min(cache_timestamps, key=cache_timestamps.get)
            model_cache.pop(oldest, None)
            signal_cache.pop(oldest, None)
            cache_timestamps.pop(oldest, None)
        model_cache[key] = {"model": model, "scaler": scaler}
...
def _cache_get(key):
    ...
    with cache_lock:
        ts = cache_timestamps.get(key, 0)
        if time.time() - ts > CACHE_TTL_SECONDS:
            model_cache.pop(key, None)
            signal_cache.pop(key, None)
            cache_timestamps.pop(key, None)
            return None, None
        return model_cache.get(key), signal_cache.get(key)
...
def _cache_clear_expired():
    ...
    now = time.time()
    with cache_lock:
        expired = [k for k, ts in cache_timestamps.items() if now - ts > CACHE_TTL_SECONDS]
        for k in expired:
...
def get_cache(key):
    """Get cached model, scaler, and signal data as dict."""
    model_data, signal_data = _cache_get(key)
    if model_data and signal_data:
        return {"model": model_data["model"], "scaler": model_data["scaler"], "data": signal_data}
    return None


# ── Thread-safe model inference ────────────────────────────────
def _predict_safe(model, scaler, X):
    """Thread-safe prediction with lock."""
    with model_lock:
...
@retry(
    ...
    retry=retry_if_exception_type((req.RequestException, req.Timeout, ValueError)),
    reraise=True
)
...
def load_data(symbol="BTCUSDT", interval="5m", limit=500):
    try:
        ...
        log.warning(f"[Binance] {e} — trying backup")
        try:
            return _fetch_cryptocompare(symbol, limit)
        except Exception as e2:
            log.error(f"[Backup] {e2}")
            return pd.DataFrame()
...
@retry(
    ...
    stop=stop_after_attempt(3),
    retry=retry_if_exception_type((req.RequestException, req.Timeout)),
    reraise=True
)
def fetch_fear_greed():
    try:
        r = req.get(FEAR_GREED_URL, params={"limit": 1}, timeout=REQUEST_TIMEOUT)
        r.raise_for_status()
        d = r.json()["data"][0]
        return {"value": int(d["value"]), "label": d["value_classification"]}
    except Exception as e:
        log.warning(f"[FearGreed] {e}")
...
def backtest(df, model, scaler, conf_thr=0.60, sl=0.008, tp=0.016, fee=0.001):
    ...
    Xs    = scaler.transform(X)
    preds, probas = _predict_safe(model, scaler, X)
...
def predict_signal(df, model, scaler):
    ...
    pred, proba = _predict_safe(model, scaler, X)
    proba = proba[0]
    pred  = int(pred[0])
...
def ready():
    ...
    global SERVER_READY
    deps_ok = HAS_NUMPY and HAS_PANDAS and HAS_SKLEARN
    cache_warmed = len(model_cache) > 0 or not SERVER_READY  # First request warms cache
    ready = deps_ok and (cache_warmed or SERVER_READY)
    SERVER_READY = True
    return ok({
        "ready": ready,
...
def signal_route():
    ...
    try:
        ...
        if cached:
            ...
            sig  = predict_signal(df, cached["model"], cached["scaler"])
...
if __name__ == "__main__":
    ...
    log.info("=" * 50)
    log.info("  CryptoScalp AI - Backend v2")
    log.info("  -----------------------------")
    log.info(f"  numpy:        {'OK' if HAS_NUMPY else 'MISSING -> pip install numpy'}")
    log.info(f"  pandas:       {'OK' if HAS_PANDAS else 'MISSING -> pip install pandas'}")
    log.info(f"  scikit-learn: {'OK' if HAS_SKLEARN else 'MISSING -> pip install scikit-learn'}")
    log.info("")
    log.info(f"  API      ->  http://{HOST}:{PORT}")
    log.info(f"  Health   ->  http://{HOST}:{PORT}/api/health")
    log.info(f"  Ready    ->  http://{HOST}:{PORT}/api/ready")
    log.info(f"  Cache TTL: {CACHE_TTL_SECONDS}s, Max entries: {MAX_CACHE_ENTRIES}")
